...
func main() {
	...
	r.GET("/movies/:id", func(c *gin.Context) {
		...
		tx := db.Db.Preload("Directors").Preload("Cast").Preload("Countries").Preload("Genres").First(&movie, id)
	...
	r.GET("/movies/bytitle/:title", func(c *gin.Context) {
		...
		tx := db.Db.Preload("Directors").Preload("Cast").Preload("Countries").Preload("Genres").Where("Title LIKE ?", "%"+title+"%").Find(&movies)
